...
/**
 * 시세 요약. RE:VISION 홈페이지의 data/market.json 과 같은 모양(groups → items{name,value,delta,dir})으로 내보낸다.
 * 홈페이지 ticker가 이 주소를 읽으면 수동 갱신이 자동으로 바뀐다.
 * delta는 크기만(부호·화살표 없음), 방향은 dir("up" | "dn")로만 낸다. 값이 없으면 delta는 빈 문자열.
 */
...
const DASH = "–";
const fmt = (v: number | null, d: number, unit = "") => (v == null || !Number.isFinite(v) ? DASH : `${v.toLocaleString("ko-KR", { minimumFractionDigits: d, maximumFractionDigits: d })}${unit}`);
/** 전년비 등 변화량. null → "", 0 → "0.0%", 그 외 크기만 + unit. 방향은 dir 필드 */
const delta = (v: number | null, unit = "%"): { delta: string; dir?: "up" | "dn" } =>
  v == null || !Number.isFinite(v) ? { delta: "" } : { delta: `${Math.abs(v).toFixed(1)}${unit}`, dir: v > 0 ? "up" : v < 0 ? "dn" : undefined };
...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json({ updatedAt: new Date(Date.now() + 9 * 3600 * 1000).toISOString().slice(0, 10), source: "RE:LAB", groups }, {
